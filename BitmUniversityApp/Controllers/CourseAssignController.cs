 ...
 */
        ...
        {
            //var courses = db.Courses.Where(m => m.DepartmentId == deptId).OrderBy(s => s.Department.DeptCode).ToList();
            List<Course> courses = (from q in db.Courses where q.DepartmentId == deptId orderby q.CourseName ascending select q).ToList();
            return Json(courses, JsonRequestBehavior.AllowGet);
        }
